...
Hook with no arguments to access some useful values from the [AdminContext](#admincontext). It returns an object.
...
- `previewMode` is `true` when the Full-screen preview is active in the Admin dashboard and `false` otherwise.
