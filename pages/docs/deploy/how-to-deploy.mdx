...
**ReactBricks lets the Admin set up two deploy hooks** for an App (one for the Production environment and one for the Staging environment). They can be set from the App Settings page of the Admin or from the React Bricks Dashboard. The Admin can then give to the Editors the permission to trigger a Staging and/or Production deploy, simply clicking on **Deploy to Staging** or **Deploy to Production** from the Admin interface.
