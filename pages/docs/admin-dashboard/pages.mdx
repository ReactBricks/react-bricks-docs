...
A page has its content, but it has also a type, name, slug, author, meta values (title, description, language, featured image, reading time), author, status, lock status, tags and creation date.

[See the `Page` type](/api-reference/types/types#page)
