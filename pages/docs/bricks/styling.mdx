...
Styled-components by default inject styles in the CSSOM in production: this prevents React Bricks from correctly inject styles in the content bricks.

Luckily the `StyleSheetManager` component, from Styled-components v5, has the `disableCSSOMInjection` property, which switches to the text-node injection system.
