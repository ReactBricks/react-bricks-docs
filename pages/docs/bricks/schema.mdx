...
| `hideFromAddMenu` | If true, the component isn't shown in the list of components available in the "add block" menu. For example, you may want to hide a block that can be use only inside of a [<Repeater />](/api-reference/visual-components/repeater). |
