...
| `newItemMenuOpen`        | If `true` the "Add new..." accordion menu is open by default; if `false` it is closed by default; otherwise, by default it is open when the number of `repeaterItems` is <= 4 and closed otherwise.                                                                                                            |
