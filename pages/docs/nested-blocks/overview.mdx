...
- Set `newItemMenuOpen` to `true` or `false` on the brick's schema to decide if you want the "Add new..." menu open or closed by default. If you don't set this value, the default is open, if the number of repeaterItems is less or equal to 4, closed otherwise.
- Set [default props](get-default-props) for the repeating items to have an initial set of nested blocks when a new block is created
