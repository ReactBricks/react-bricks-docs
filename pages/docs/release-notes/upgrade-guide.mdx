---
title: Upgrade guide
description: Learn how to migrate from React Bricks v1.0.68 to v2.0.0
---
...
**To improve the performance where no CSS-in-JS is used**, the action needed to support CSS in JS are triggered only if this flag is set to true.
...
``` jsx
<Repeater propName="tags" renderWrapper={...} itemProps={...} /> 
```
...
### Repeater component:
The **optional** `renderWrapper` functional component is rendered if there is at least one repeated item.  
...
On **Text**, **RichText** and **Image** components, remove `value` and `onChange`.
